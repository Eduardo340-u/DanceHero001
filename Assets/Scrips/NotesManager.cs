...
public class NotesManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnNotes()
    {
        while (_currentNoteChart.notes.Count > 0)
        {
            ...
            List<NoteData> notesToSpawn = _currentNoteChart.notes.FindAll(note => note.spawnTime <= _timer);
            foreach (NoteData noteData in notesToSpawn)
            {
                _currentNoteChart.notes.Remove(noteData);
                if (noteData.laneIndex < 0 || noteData.laneIndex >= _laneManager.GetLaneCount())
                {
                    Debug.LogWarning("Invalid lane index: " + noteData.laneIndex);
                    continue;
                }
                Lane currentLane = _laneManager.GetLane(noteData.laneIndex);
                GameObject noteObject = Instantiate(currentLane.NotePrefab, currentLane.NotesPivot.position, Quaternion.identity);
